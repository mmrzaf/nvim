...
function M.run_just(args)
  ...
  ensure_log_win()
  append_to_log({ "", "▶ Running: just" .. (args and #args > 0 and (" " .. table.concat(args, " ")) or ""), "────────────────────" })
  ...
  local job_id = vim.fn.jobstart(cmd, {
    ...
    on_stdout = function(_, data, _)
      if data then
        vim.schedule(function() append_to_log(data) end)
      end
    end,
    ...
    on_exit = function(_, code, _)
      M.procs[job_id] = nil
      vim.schedule(function()
        append_to_log({ "────────────────────", "just exited with code: " .. tostring(code), "" })
      end)
    end,
  })
...
function M.setup(opts)
  ...
  vim.keymap.set("n", "<leader>j", function() M.run_just() end, { desc = "Run just" })
  vim.keymap.set("n", "<leader>jj", function()
    -- prompt for args and run: e.g. "build --verbose"
    local input = vim.fn.input("just args: ")
    if input ~= nil then
      local parsed = {}
      for token in string.gmatch(input, "%S+") do table.insert(parsed, token) end
      M.run_just(parsed)
    end
